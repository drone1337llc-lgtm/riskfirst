...
**Shipped:** **89 passing offline tests** — 27 cryptobot + 62 options (Greeks, parity, IV recovery, 2% sizing, drawdown blocks, spread/OI screens, MCP contract parsing, LLM referee, paper-loop runner). Zero keys, zero network. Paper-legal, Level-3-only: covered calls, cash-secured puts, protective-put/collar. **No naked shorts.**
...
**Hook:** "89 tests on an options agent that run offline with zero keys. Here's how."

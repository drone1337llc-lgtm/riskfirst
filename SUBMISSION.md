...
2. **Built a walk-forward OOS gate** (`eval_oos.py`, 75/25 train/eval windows, 4 folds, ~300k timesteps) — the crypto lane stays only if out-of-sample Sharpe is positive **net**. Spec-compliant smoke: mean annualized Sharpe **13.37**, **0/4 negative folds**; full 300k run results land in the repo when complete.
...
- OOS walk-forward gate (75/25 window, 4 folds, 300k timesteps): smoke mean ann. Sharpe **13.37**, **0/4 negative folds** (full run in flight — updated in repo).

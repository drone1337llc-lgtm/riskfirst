...
The MCP route is live (uvx alpaca-mcp-server, stdio/http/sse, `bin/run_mcp.sh`). The agent doesn't fake orders — it talks to Alpaca through typed tools, the same ones a human CLI user would.
...
- **$500/leg notional cap** — no yoloing the paper account
- **$1 minimum premium** — no dust orders
- **Position intent validation** — BTO/BTC/STO/STC must be coherent before the broker sees it
- **−10% trailing circuit-breaker** — the book goes flat, full stop

Everything is smoke-tested pre-broker (over-leveraged and bad-intent requests rejected in tests, never reaching the API).
